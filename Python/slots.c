...
void
_PySlotIterator_Init(_PySlotIterator *it, void *slots,
    ...
    if (info->reject_duplicates || info->deprecate_duplicates) {
        if (_PySlotIterator_SawSlot(it, id)) {
            ...
            if (info->reject_duplicates) {
                ...
                PyErr_Format(
                    PyExc_SystemError,
                    "%s%s%s has more than one Py_%s (%d) slot",
                    kind_name(it->kind),
                    it->name ? " " : "",
                    it->name ? it->name : "",
                    info->name,
...
_PySlotIterator_Next(_PySlotIterator *it)
{
    ...
    while (true) {
        ...
        if (result->sl_id == Py_slot_end) {
            ...
            const uint16_t bad_flags = PySlot_HAS_FALLBACK | PySlot_STATIC;
            if (flags & bad_flags) {
