...
{
#if Py_GIL_DISABLED
    // Check that this def does not come from a pre-free-threading stable ABI
    // extension.
    //
    ...
    // change some of the internals, this check can go.
    //
    ...
    // (he values may change at runtime as the PyModuleDef is used, but
    ...
    if ((flags & bits) != _Py_STATICALLY_ALLOCATED_FLAG) {
        const char *message = "invalid PyModuleDef, extension possibly "
            "compiled for non-free-threaded stable ABI";
