...
/* An array of data corresponding to Py_tp_* constants,
  * for use in e.g. PyType_Spec and PyType_GetSlot.
  * Each entry has:
  * - two offsets: "slot_offset" and "subslot_offset".
  ...
  *   that struct.
  * - name of the slot (for error messages)
  * - some flags
  *
  * The actual table is generated by a script.
  */

typedef struct PySlot_Info {
    Py_ssize_t subslot_offset;
    Py_ssize_t slot_offset;
    char *name;
    bool apply_replace :1;
} PySlot_Info;

static const PySlot_Info pyslot_infos[] = {
    {0, 0},
...
replace_slot(PyHeapTypeObject *res, const PyType_Slot *slot) {
    ...
    short slot_offset = slotinfo->slot_offset;
    if (slotinfo->subslot_offset == -1) {
        /* Set a slot in the main PyTypeObject */
        *(void**)((char*)res_start + slot_offset) = slot->pfunc;
    }
    else {
        void *procs = *(void**)((char*)res_start + slot_offset);
        short subslot_offset = slotinfo->subslot_offset;
    ...
    PyType_Spec *spec, PyObject *bases_in)
{
    ...
    for (slot = spec->slots; slot->slot; slot++) {
        if (slot->slot < 0
            || (size_t)slot->slot >= Py_ARRAY_LENGTH(pyslot_infos)) {
...
PyType_GetSlot(PyTypeObject *type, int slot)
{
    ...
    int slot_offset = pyslot_infos[slot].slot_offset;
    ...
    if (pyslot_infos[slot].subslot_offset == -1) {
        return parent_slot;
    }
    return *(void**)((char*)parent_slot + pyslot_infos[slot].subslot_offset);
}
...
PyType_ApplySlots(PyTypeObject *type, PyType_Slot *slot, Py_ssize_t count_in)
{
    ...
    if (count_in < -1) {
        // (note that -1 is valid)
        PyErr_Format(PyExc_SystemError,
                        "PyType_ApplySlots: bad count: %zd",
                        count_in);
        return -1;
    }
    int slots_len = Py_ARRAY_LENGTH(pyslot_infos);
    ...
    for (; slot->slot && (count > 0); slot++, count--) {
        ...
        if ((slot->slot < 0) || (slot->slot >= slots_len)) {
            PyErr_Format(PyExc_SystemError,
                         "PyType_ApplySlots: bad slot: %d",
                         slot->slot);
            return -1;
        }
        const PySlot_Info *slotinfo = &pyslot_infos[slot->slot];
