...
def generate_typeslots(out=sys.stdout, aux_out=None):
    ...
    if aux_out:
        print(f'0, "slot_end", any', file=aux_out)
        for i in range(1,M):
            print(f'{i}, "{names[i]}", type', file=aux_out)


def main():
    if len(sys.argv) == 3:
        with open(sys.argv[1], "w") as f,  open(sys.argv[2], "w") as ff:
            generate_typeslots(f, ff)
    elif len(sys.argv) == 2:
