...
{
    char *inpath, *outpath, *code_name = NULL;
    ...
    code_name = malloc(100);
    if (code_name == NULL) {
        fprintf(stderr, "could not allocate memory for module name\n");
        goto error;
    }
    if (snprintf(code_name, 100, "<frozen %s>", argv[3]) < 0) {
        strcpy(code_name, "<frozen module>");
    }
    code_name[98] = '>';
    code_name[99] = 0;
    ...
    fprintf(outfile, "%s\n", header);
    if (is_bootstrap)
        fprintf(outfile, "const unsigned char _Py_M_%s[] = {\n", argv[3]);
    else
        fprintf(outfile,
                "const unsigned char _Py_M_%s[] = {\n", argv[3]);
