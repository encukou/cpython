...
@generator("doc_list", 'Doc/data/stable_abi.txt')
...
@generator("doc_page", 'Doc/c-api/_stable_abi.rst')
def gen_doc_page(manifest, args, outfile):
    """Generate/check the documentation page for stable ABI"""
    write = partial(print, file=outfile)
    write(".. Generated by Tools/scripts/stable_abi.py")
    write()
    roles = {
        ...
        'struct': 'type',
        'macro': 'macro',
        # 'const': 'const',  # XXX: all undocumented
        'typedef': 'type',
    }
    for item in manifest.select(roles.keys(), include_abi_only=False):
        write(f'* :c:{roles[item.kind]}:`{item.name}`')
...
@generator("ctypes-test", 'Lib/test/test_stable_abi_ctypes.py')
def gen_python3dll(manifest, args, outfile):
    """Generate/check the test that check symbol availability using ctypes"""
    write = partial(print, file=outfile)
    write(textwrap.dedent(r"""
        # Generated by Tools/scripts/stable_abi.py
        ...
        _ABI_SYMBOLS = (
            # Note that this list includes symbols that are not part
            # of the limited API.
    """), file=outfile)
    for item in manifest.select({'function', 'data'}, include_abi_only=True):
        if item.ifdef:
            # XXX: test build-specific symbols as well
            continue
        write(f'    "{item.name}",')
    write(textwrap.dedent(r"""
        )
        ...
        class AbiTest(unittest.TestCase):
            def test_symbols(self):
                for symbol_name in _ABI_SYMBOLS:
                    with self.subTest(name=symbol_name):
                        getattr(ctypes.pythonapi, symbol_name)
    """), file=outfile)
