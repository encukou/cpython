...
@generator("pc_python3_def", 'PC/python3.def')
def gen_pc_python3_def(manifest, args, outfile):
    """Generate/check the stable ABI list for documentation annotations"""
    write = partial(print, file=outfile)
    write('; Generated by Tools/scripts/stable_abi.py')
    write()
    write('; This file specifies the import forwarding for python3.dll')
    write('; It is here for projects that re-export these symbols.')
    write('; It is not used in Python itself, so it is not tested.')
    write()
    write('LIBRARY "python3"')
    write('EXPORTS')
    for item in manifest.select({'function', 'data'}):
        if item.ifdef and not manifest.contents[item.ifdef].windows:
            continue
