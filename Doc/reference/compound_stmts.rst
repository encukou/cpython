...
.. grammar-snippet:: function_def decorators parameters slash_no_default default
   ...
   parameters: ( ((`slash_no_default` `param_no_default`* | `slash_with_default` | `param_no_default`+) `param_with_default`* | `param_with_default`+) [`star_etc`]
     : | `star_etc`
     : )
   ...
   star_etc: ( '*' ((`param_no_default` | NAME ':' `star_expression` (',' | &')')) `param_maybe_default`* | ',' `param_maybe_default`+) [`kwds`]
     : | `kwds`
     : )
